...
def build_model(cfg):
    ...
    elif torso_cfg["name"] == "LayerNormMLP":
        hidden_layers = tuple(torso_cfg["hidden_layers"])
        activation = getattr(torch.nn, torso_cfg["activation"])
        critic_torso = models.LayerNormMLP(hidden_layers, activation)
    else:
        raise ValueError('\n Critic TORSO is not correctly assinged \n')
    ...
    elif critic_cfg["head"]["name"] == "GaussianMixtureHead":
        critic_head = models.GaussianMixtureHead(num_dimensions=critic_cfg["head"]["num_dimensions"],\
                                                 num_components=critic_cfg["head"]["num_components"],\
                                                 init_scale=critic_cfg["head"]["init_scale"])                                             
    else: 
...
def build_critic_updater(cfg):
    ...
    if critic_cfg["name"] == "DiffusionExpectedSARSA":
    ...
    elif critic_cfg["name"] == "DiffusionMixtureOfGaussian":
        ...
        return updaters.DiffusionMixtureOfGaussian(
            num_action_samples=critic_cfg["num_action_samples"],num_value_samples=critic_cfg["num_value_samples"] ,\
            evaluate_stochastic_policy = critic_cfg["evaluate_stochastic_policy"],optimizer=critic_optimizer,\
            gradient_clip=critic_cfg["gradient_clip"] )
