...
from tonic.torch.models.encoders import ObservationActionEncoder,ObservationEncoder
...
def build_model(cfg):
    ...
    elif cfg["actor"]["encoder"]["name"] == "ObservationEncoder":
        actor_encoder = ObservationEncoder()
    else:
        ValueError("\n Check Assigned Encoder for Actor in builder or yaml file \n")
    ...
    # Head
    if critic_cfg["head"]["name"] == "DistributionalValueHead":
...
def build_critic_updater(cfg):
    ...
    if critic_cfg["name"] == "DistributionalDeterministicQLearning":
        # Get optimizer
